...
| P1 | Organizing an ABAP project repo for GitHub Copilot — the `.github/` folder: rules, scoped rules, agents, skills, prompts | AI | S02 slot reassigned to this; S01 said "copy these files in" without teaching the folder, so nobody can build or adapt one. Also corrects S01: prompt files are deprecated and unsupported in Eclipse at every version, while custom agents became Eclipse-supported at extension 0.13.0 | M | **Scheduled — S02, `2026-09-25`** |
| P1 | RAP managed BO end-to-end for classic ABAP developers | Practice | Core skill for the greenfield build; recurring review gaps in save-sequence use. Content already drafted — bumped from S02 to make room. | M | **Proposed for S03, `2026-10-09`** — draft at `docs/coe-sessions/session-rap-managed-bo.md` |
| P1 | Reading a released-API contract in ADT (API state, successor, deprecation) | Practice | Prevents non-released usage getting into transports; closes S01's "AI authorship ≠ compliance" rule. Content already drafted — bumped from S03. | S | Ready — draft at `docs/coe-sessions/session-released-api-contracts.md` |
| P1 | ABAP AI SDK / ISLM: building an AI scenario into an L2C app | AI | Standard delivery on our PCE 2025 and **BTP + AI Core entitlement is available** — so this is buildable. Comm scenario `SAP_COM_0A69`, SAP AI Core. Flips S01: AI *inside* our ABAP, not just writing it. | L | Proposed — likely S05, exact slot depends on where the released-API contract session (also drafted, currently unscheduled) lands |
