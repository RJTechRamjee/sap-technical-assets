...
Input: a **Technical Design Document** (`templates/technical-design-document.md`
§4–§6 — object inventory, data model, behavior design) or a described
transactional entity. When a TDD exists, treat its object inventory and rule
table as the contract: generate exactly those objects with those names, and flag
anything the TDD didn't specify rather than inventing it.
...
5. **Draft table** note (if draft-enabled) — the draft table is a *database
   table*, generated by the ADT quick-fix from the behavior definition, not a CDS
   view and not hand-written. State its name per project convention and the
   `total etag` field; don't emit a source block for it.
...
7. **DCL** — only when row-level authorization applies; base it on a released
   authorization CDS or aspect. Note that many projects name the access control
   after the view it protects rather than with a `ZDCL_` prefix; follow whichever
   `reference/sap-project-standards.md` §5 states, and say which you used.
